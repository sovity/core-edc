...
public class DspCatalogApiEndToEndTest {
    ...
    void shouldPermitPaginationWithLinkHeader(String basePath, JsonLdNamespace namespace) {
        ...
        policyDefinitionStore.create(PolicyDefinition.Builder.newInstance().policy(Policy.Builder.newInstance().build()).participantContextId("anonymous").build())
                .onSuccess(policy -> {
                    var contractDefinition = ContractDefinition.Builder.newInstance().accessPolicyId(policy.getId()).contractPolicyId(policy.getId()).participantContextId("anonymous").build();
